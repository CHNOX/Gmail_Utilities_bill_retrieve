...
Recupera **tutte le email** ricevute dal mittente **Enel Energia** (`enelenergia@enel.sandsiv.com`) dalla tua casella Gmail e genera un file **Excel** con data, oggetto e anteprima del testo, ordinate dalla più recente.

## Cosa produce

Il file Excel generato (`enel_emails_YYYYMMDD_HHMMSS.xlsx`) contiene:

| # | Data | Oggetto | Anteprima |
|---|------|---------|-----------|
| 1 | 15/04/2026 08:32 | La tua bolletta di aprile | Gentile cliente, la informiamo che… |
| 2 | 14/03/2026 09:10 | Riepilogo consumi marzo | In allegato trovi il dettaglio… |
| … | … | … | … |
...
1. Si aprirà **automaticamente il browser** con la schermata di consenso Google
2. Google potrebbe mostrare l'avviso **"App non verificata"** — è normale per app personali in fase di test:
   - Clicca **"Avanzate"**
   - Clicca **"Vai a [nome app] (non sicuro)"**
3. Concedi i permessi di lettura email
4. Il browser mostrerà `The authentication flow has completed` — puoi chiuderlo
5. Lo script inizia la ricerca e mostra il progresso nel terminale
...
=======================================================
   Gmail Utilities Bill Retrieve
   Mittente: enelenergia@enel.sandsiv.com
=======================================================
...
Ricerca email da: enelenergia@enel.sandsiv.com
  ...
  Batch 1: +24 messaggi (totale recuperati: 24)

Recupero completato: 24 email trovate.

File Excel salvato: enel_emails_20260418_103215.xlsx

Prime 10 email (più recenti):
-------------------------------------------------------
   1. [18/04/2026] La tua bolletta di aprile 2026
   2. [15/03/2026] La tua bolletta di marzo 2026
   3. [14/02/2026] La tua bolletta di febbraio 2026
   ...
...
Verifica che il mittente `enelenergia@enel.sandsiv.com` abbia effettivamente inviato email alla casella autenticata. Puoi cercarlo manualmente in Gmail con `from:enelenergia@enel.sandsiv.com`.
